...
| `/dashboard/{repo_full}?tab=artifacts` | dashboard-repo | authenticated | active | dashboard-eligible user | render artifact topology and registry | open flagged change / review stored evidence | artifact graph, registry, and stored-evidence actions must hydrate | repo dashboard APIs | slice-01 browser wait check on `doria90/dummyAI?tab=artifacts` | pass |  |  | rendered artifact topology, stored-evidence status, and artifact registry after network idle |
