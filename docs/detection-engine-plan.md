...
- Phase D roadmap work now includes disabled-by-default scenario and hybrid-analysis planning seams plus shadow-only seeded scenario execution with persisted run summaries, so optional deeper analysis stays auditable, bounded, and off in production until explicitly promoted
