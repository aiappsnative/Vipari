...
def main():
    ...
    inline_private_key = (os.getenv('GITHUB_APP_PRIVATE_KEY') or '').replace('\\n', '\n')
    if not app_id or (not priv_path and not inline_private_key):
        print('Missing GITHUB_APP_ID and GitHub App private key material, skipping GitHub App test')
    else:
        try:
            if inline_private_key:
                pk = inline_private_key
            else:
    ...
    if provider == 'openai':
        if not openai_key:
            print('Missing OPENAI_API_KEY, skipping')
        else:
            print('OpenAI provider configured')
    elif provider == 'foundry':
        if not endpoint or not foundry_key:
            print('Missing AZURE_OPENAI_ENDPOINT or FOUNDRY_API_KEY, skipping')
        else:
        ...
        print('OpenAI provider inferred from OPENAI_API_KEY')
    else:
        print('Missing provider-specific AI credentials, skipping')
