...
from services.operational_policy_records import upsert_repo_policy_override, upsert_workspace_policy
...
def _bind_repo_to_workspace(
    ...
    *,
    ...
    workspace = create_workspace(db_path, slug=f"ws-{installation_id}", display_name="Worker Workspace", billing_owner_user_id=owner.id)
    upsert_github_installation(
        db_path,
    ...
    allocation = allocate_repo_to_workspace(
        ...
        workspace_id=workspace.id,
        installation_id=installation_id,
        repo_github_id=f"repo-{installation_id}",
        repo_full=repo_full,
        baseline_mode="default_branch",
        activated_by_user_id=owner.id,
    )
    update_workspace_pr_feedback_mode(db_path, workspace.id, pr_feedback_mode=workspace_mode)
    if repo_mode is not None:
        update_repo_allocation_pr_feedback_mode(db_path, allocation.id, pr_feedback_mode=repo_mode)
    return owner, workspace, allocation
...
def test_worker_persists_shadow_scenario_eval_plan_metadata(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: 9001)
...
def test_worker_persists_effective_policy_provenance_for_allocated_repo(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    _, workspace, _ = _bind_repo_to_workspace(db_path, workspace_mode="comments")
    job = create_audit_job(
        db_path,
        repo_full="doria90/dummyAI",
        pr_number=96,
    ...
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: 9010)
    monkeypatch.setattr("services.audit_worker.sync_pr_label", lambda *args, **kwargs: None)
                ...
                create=lambda **kwargs: SimpleNamespace(
                    choices=[
                        SimpleNamespace(
                            message=SimpleNamespace(
                                content=(
                                    "Summary: The prompt changed and should be reviewed.\n"
                                    "Risk Level: Medium\n"
                                    "Confidence: High\n"
                                    "Detailed Analysis:\n"
                                    "- The prompt now permits more disclosure behavior.\n"
    ...
    audit = get_pull_request_audit_for_job(db_path, job.id)
    assert audit is not None
    assert audit.workspace_policy_version_id is not None
    assert audit.repo_policy_version_id is None
    assert audit.effective_policy_source == "workspace"
    assert audit.effective_policy_hash

    workspace_policy = get_workspace_policy(db_path, workspace.id)
    assert workspace_policy is not None
    assert workspace_policy.active_version_id == audit.workspace_policy_version_id

    policy_decision = json.loads(audit.policy_decision_json)
    assert policy_decision["effective_policy_source"] == "workspace"
    assert policy_decision["workspace_policy_version_id"] == audit.workspace_policy_version_id
    assert policy_decision["repo_policy_version_id"] is None
    assert policy_decision["effective_policy"]["preset_key"] == "balanced"


def test_worker_skips_semantic_review_when_workspace_policy_disables_it(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    ...
    upsert_workspace_policy(
        ...
        policy=normalize_operational_policy(
            {
                "llm_strategy": {
                    "when_to_run_semantic": "never",
                    "when_to_run_verifier": "never",
                }
            }
        ),
    ...
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: posted.append(args[3]) or 9011)
    monkeypatch.setattr("services.audit_worker.sync_pr_label", lambda *args, **kwargs: None)
    ...
    assert invoked == []
    assert posted
    assert "Operational policy skipped semantic model review" in posted[0]

    audit = get_pull_request_audit_for_job(db_path, job.id)
    assert audit is not None
    assert audit.semantic_review_completed is False
    assert audit.verifier_request_count == 0
    assert json.loads(audit.policy_decision_json)["effective_policy"]["llm_strategy"]["when_to_run_semantic"] == "never"


def test_worker_honors_repo_policy_verifier_strategy(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    owner, workspace, allocation = _bind_repo_to_workspace(db_path, workspace_mode="comments")
            ...
            return SimpleNamespace(
                choices=[
                    SimpleNamespace(
                        message=SimpleNamespace(
                            content=(
                                "Summary: The prompts changed, but reviewer certainty is limited.\n"
                                "Risk Level: Low\n"
                                "Confidence: Low\n"
                                "Recommendation: Review the changed AI control surface closely before merge."
                            )
    ...
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: posted.append(args[3]) or 9012)
    monkeypatch.setattr("services.audit_worker.sync_pr_label", lambda *args, **kwargs: None)
    ...
    assert audit.verifier_request_count == 0
    assert audit.verifier_trigger is None
    assert json.loads(audit.policy_decision_json)["effective_policy"]["llm_strategy"]["when_to_run_verifier"] == "never"
...
def test_worker_persists_shadow_hybrid_analysis_plan_metadata(tmp_path, monkeypatch):
    ...
    assert audit.hybrid_analysis_request_count == 1
...
def test_worker_posts_request_changes_review_when_feedback_mode_reviews_and_risk_high(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    _bind_repo_to_workspace(db_path, workspace_mode="reviews")
    job = create_audit_job(
    ...
    assert reviews[0][1] == "REQUEST_CHANGES"
    assert labels == [True]
    ...
    comment = get_audit_comment_for_audit(db_path, audit.id)
    assert comment is not None
    assert comment.github_comment_id is None
    assert comment.github_review_id == 1111
    assert comment.comment_mode == "review_request_changes"


def test_worker_posts_comment_review_when_feedback_mode_reviews_and_risk_medium(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    _bind_repo_to_workspace(db_path, workspace_mode="reviews")
    job = create_audit_job(
        ...
        installation_id=123,
        head_sha="sha-12",
        diff_text="diff --git a/prompts/policy.md b/prompts/policy.md\nindex 1..2\n@@ -0,0 +1 @@\n+Keep explanations concise and cite the approved workflow.\n",
    )
    reviews = []

    monkeypatch.setattr(
        "services.audit_worker.build_llm_comment",
        lambda *args, **kwargs: "Summary: Control surface expanded.\nRisk Level: Medium\nConfidence: High\nRecommendation: Review before merge.",
    )
    monkeypatch.setattr("services.audit_worker.generate_jwt", lambda *args, **kwargs: "jwt")
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    ...
    comment = get_audit_comment_for_audit(db_path, audit.id)
    assert comment is not None
    assert comment.github_review_id == 1212
...
def test_worker_suppresses_github_output_when_feedback_mode_off(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    owner = create_user(db_path, display_name="Owner", primary_email="owner@example.com")
    workspace = create_workspace(db_path, slug="ws-off", display_name="Workspace Off", billing_owner_user_id=owner.id)
    upsert_github_installation(
        db_path,
        workspace_id=workspace.id,
        installation_id=123,
    ...
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: posted.append(args) or 3100)
...
def test_worker_silent_off_mode_persists_fallback_without_github_output(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs.db")
    init_db(db_path)
    owner = create_user(db_path, display_name="Owner", primary_email="owner@example.com")
    workspace = create_workspace(db_path, slug="ws-off-fallback", display_name="Workspace Off Fallback", billing_owner_user_id=owner.id)
    upsert_github_installation(
        db_path,
        workspace_id=workspace.id,
        installation_id=123,
    ...
    update_workspace_pr_feedback_mode(db_path, workspace.id, pr_feedback_mode="comments")
    update_repo_allocation_pr_feedback_mode(db_path, allocation.id, pr_feedback_mode="off")
    ...
    monkeypatch.setattr("services.audit_worker.get_installation_token", lambda *args, **kwargs: "token")
    monkeypatch.setattr("services.audit_worker.fetch_file_content", lambda *args, **kwargs: "artifact snapshot")
    monkeypatch.setattr("services.audit_worker.upsert_pr_comment", lambda *args, **kwargs: posted.append(args) or 3200)
    monkeypatch.setattr("services.audit_worker.sync_pr_label", lambda *args, **kwargs: labels.append(args))
    monkeypatch.setattr("services.audit_worker.build_llm_comment", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("model unavailable")))
...
def test_worker_executes_scenario_and_hybrid_once_in_comment_mode(tmp_path, monkeypatch):
    db_path = str(tmp_path / "jobs-single-execution.db")
    init_db(db_path)
    owner, workspace, _allocation = _bind_repo_to_workspace(db_path, workspace_mode="comments")
    upsert_entitlement(
        db_path,
        ...
        or ScenarioEvalExecutionSummary(
            ...
            executions=(
                ScenarioEvalExecution(
                    scenario_key="dummyai-review-target",
                    artifact_paths=("prompts/policy.md",),
                    package_path="artifacts/eval-runs/pr-81/run-package.json",
                    comparison_path="artifacts/eval-runs/pr-81/comparison-summary.json",
                    assertion_summary={"all_passed": True, "failed_count": 0},
                    candidate_source="seeded",
                ),
            ),
...
def test_worker_active_verifier_skips_second_pass_when_budget_is_exhausted(tmp_path, monkeypatch):
    ...
    assert "workspace " not in posted[0].lower()
